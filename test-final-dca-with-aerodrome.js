...
async function testFinalDCAWithAerodrome() {
  ...
  try {
    ...
    console.log('💰 Current USDC balance:', (Number(usdcBalance) / 1e6).toFixed(6), 'USDC');
    ...
    console.log('   Smart wallet USDC:', (Number(usdcBalance) / 1e6).toFixed(6), 'USDC');
    console.log('   User wallet SPX:', (Number(initialSpxBalance) / 1e8).toFixed(8), 'SPX');
    
    // Execute DCA
    console.log('\\n4️⃣ Executing DCA with Aerodrome...');
    const executionResponse = await fetch(`http://localhost:3000/api/test-force-dca-execution?orderId=${orderId}`);
    ...
    if (executionResult.success && executionResult.result.success) {
      ...
      console.log('\\n5️⃣ Final balances:');
      console.log('   Smart wallet USDC:', (Number(finalUsdcBalance) / 1e6).toFixed(6), 'USDC');
      console.log('   User wallet SPX:', (Number(finalSpxBalance) / 1e8).toFixed(8), 'SPX');
      console.log('   SPX gained:', ((Number(finalSpxBalance) - Number(initialSpxBalance)) / 1e8).toFixed(8), 'SPX');
      ...
      console.log('❌ DCA execution failed:', executionResult.result?.error || 'Unknown error');
