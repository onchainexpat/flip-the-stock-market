    ...
    if (data.sessionKeyApproval) {
      try {
        const approvalData = JSON.parse(
          Buffer.from(data.sessionKeyApproval, 'base64').toString('utf-8'),
        );
        ...
        console.log(
          '   Account address:',
          approvalData.accountParams?.accountAddress,
        );
        console.log(
          '   Expected smart wallet: 0x320b2943e26ccbDacE18575e7974EDC200BA4dCE',
        );
        console.log(
          '   Addresses match:',
          approvalData.accountParams?.accountAddress?.toLowerCase() ===
            '0x320b2943e26ccbDacE18575e7974EDC200BA4dCE'.toLowerCase(),
        );

        if (approvalData.privateKey && approvalData.privateKey.length > 0) {
          console.log(
            '✅ Session private key is available for deserialization',
          );
