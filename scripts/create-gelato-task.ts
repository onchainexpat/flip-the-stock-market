...
async function createGelatoTask() {
  ...
  try {
    ...
    const provider = new ethers.JsonRpcProvider(process.env.NEXT_PUBLIC_ZERODEV_RPC_URL);
    const signer = new ethers.Wallet(process.env.GELATO_DEPLOYER_PRIVATE_KEY!, provider);
    ...
    if (balance < ethers.parseEther('0.1')) {
      console.warn('⚠️  Low balance! You need at least 0.1 ETH for deployment and funding');
    }
    ...
    const taskArgs = {
      ...
      web3FunctionHash,
      web3FunctionArgsHash: ethers.keccak256(ethers.toUtf8Bytes(JSON.stringify(userArgs))),
      trigger: {
        type: 'time',
    ...
    const fundTx = await gelatoOps.depositFunds(fundAmount, '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'); // ETH
    ...
    console.log(`   Created: ${new Date(taskDetails.createdDate).toLocaleString()}`);
    ...
    console.log(`4. Task URL: https://app.gelato.network/functions/task/${taskId}:8453`);
