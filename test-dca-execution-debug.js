...
async function debugDCAExecution() {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/test-force-dca-execution?orderId=${orderId}`);
    ...
    if (data.result) {
      ...
      if (data.result.error) {
        ...
        if (data.result.error.includes('callData')) {
          console.log('\\n📋 Call data in error:', data.result.error.match(/"callData":"([^"]+)"/)?.[1]?.substring(0, 100) + '...');
        }
        
        if (data.result.error.includes('0xe9ae5c53')) {
          console.log('\\n🔍 This is a multicall operation (0xe9ae5c53 = execute batch)');
          console.log('   The session key is trying to execute a batched transaction');
        }
    ...
    const orderResponse = await fetch(`http://localhost:3000/api/dca-orders-v2?userAddress=0xC9860f5D7b80015D0Ff3E440d0f8dB90A518F7E7`);
    ...
    console.log('\\n💡 Analysis:');
    console.log('   - Session keys are working correctly (authentication passes)');
