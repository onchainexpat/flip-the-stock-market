...
async function testDCATransferOnly() {
  ...
  try {
    ...
    const ordersResponse = await fetch(
      `http://localhost:3000/api/dca-orders-v2?userAddress=${USER_WALLET}`,
    );
    ...
    const latestOrder =
      ordersData.orders.find((o) => o.id === testOrderId) ||
      ordersData.orders[0];
    ...
    const agentKeyResponse = await fetch(
      `http://localhost:3000/api/debug/agent-key-data?orderId=${latestOrder.id}`,
    );
    ...
    console.log(
      '   Smart wallet:',
      (Number(newSmartBalance) / 1e6).toFixed(6),
      'USDC',
    );
    console.log(
      '   User wallet:',
