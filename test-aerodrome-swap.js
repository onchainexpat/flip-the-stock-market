...
async function testAerodromeSwap() {
  ...
  try {
    ...
    try {
      ...
      console.log('   Expected SPX output:', (Number(amountsOut[2]) / 1e8).toFixed(8), 'SPX');
      console.log('   Minimum output (1% slippage):', (Number(amountsOut[2] * 99n / 100n) / 1e8).toFixed(8), 'SPX');
    } catch (error) {
    ...
    const swapReceipt = await kernelClient.waitForUserOperationReceipt({ hash: swapTx });
    ...
    const transferReceipt = await kernelClient.waitForUserOperationReceipt({ hash: transferTx });
    ...
    console.log('   User wallet SPX:', (Number(userSpxBalance) / 1e8).toFixed(8), 'SPX');
    console.log('');
