...
async function removeAllOrders() {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/api/unified-dca-orders?userAddress=${userAddress}`,
    );
    ...
    for (const order of activeOrders) {
      ...
      console.log(
        `   Total: ${order.totalAmount} (executed: ${order.executedAmount})`,
      );
    ...
    for (const order of activeOrders) {
      try {
        ...
        const deleteResponse = await fetch(
          `${BASE_URL}/api/delete-order?orderId=${order.id}&userAddress=${userAddress}`,
          {
            method: 'DELETE',
          },
        );
        ...
        if (deleteResponse.ok && deleteData.success) {
          ...
          console.log(
            `   ❌ Failed to cancel order ${order.id}: ${deleteData.error || 'Unknown error'}`,
          );
        }
      } catch (error) {
        console.log(
          `   ❌ Error cancelling order ${order.id}: ${error.message}`,
        );
      }
    }
