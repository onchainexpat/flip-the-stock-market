  ...
  try {
    ...
    for (const key of keys) {
      ...
      if (order && order.createdAt) {
        const createdAt = Number.parseInt(order.createdAt);
    ...
    if (latestOrder) {
      console.log('Latest order:', {
        ...
        createdAt: new Date(Number.parseInt(latestOrder.createdAt)),
        agentKeyId: latestOrder.sessionKeyData
          ? JSON.parse(latestOrder.sessionKeyData).agentKeyId
          : 'N/A',
      });
...
findLatestOrder().then((orderId) => {
  if (orderId) {
    ...
    console.log(
      `curl -X POST http://localhost:3000/api/execute-gelato-dca-now -H "Content-Type: application/json" -d '{"orderId": "${orderId}"}'`,
    );
  }
  process.exit(0);
});
