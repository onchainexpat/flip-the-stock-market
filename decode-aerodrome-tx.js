const { createPublicClient, http, decodeAbiParameters, parseAbiParameters } = require('viem');
...
async function decodeTx() {
  const txHash = '0x33fd4041490253388e92be7154beabbf238ede0872a7d9ca49c0235df468cf95';
  ...
  try {
    ...
    if (functionSelector === '0x24856bc3') {
      ...
      const usdcIndex = tx.input.toLowerCase().indexOf(usdcAddress.toLowerCase());
      ...
      receipt.logs.forEach((log, index) => {
        if (log.topics[0] === '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef') {
          ...
          if (log.address.toLowerCase() === '0x833589fcd6edb6e08f4c7c32d4f71b54bda02913') {
            console.log(`     ^ USDC Transfer: ${(Number(amount) / 1e6).toFixed(6)} USDC`);
          }
          if (log.address.toLowerCase() === '0x50da645f148798f68ef2d7db7c1cb22a6819bb2c') {
            console.log(`     ^ SPX6900 Transfer: ${(Number(amount) / 1e8).toFixed(8)} SPX`);
          }
        }
      });
