...
export default function ZeroDevDCAComponent({
  ...
  const [formData, setFormData] = useState({
    amount: '10', // Start with 0.01 USDC for testing
    password: '',
  });
  ...
  const [steps, setSteps] = useState<DCAStep[]>([
    ...
    },
    {
      id: 'approve',
      title: 'Approve USDC',
      description: 'Approve smart wallet to spend USDC tokens',
      status: 'pending',
    },
    {
      id: 'swap',
      title: 'Execute Swap',
      description: 'Swap USDC for SPX tokens via OpenOcean',
      status: 'pending',
    },
    {
      id: 'transfer',
      title: 'Transfer SPX',
      description: 'Transfer SPX tokens to your wallet',
      status: 'pending',
    },
  ]);
  ...
  useEffect(() => {
    const loadBalances = async () => {
      ...
      try {
        const balances = await zerodevSmartWalletService.getWalletBalances(walletId);
  ...
  const executeDCASwap = async () => {
    ...
    try {
      ...
      if (!currentWalletId) {
        currentWalletId = await createSmartWallet();
      } else {
        updateStepStatus('setup', 'completed');
      }
      ...
      const swapAmount = BigInt(Math.floor(Number.parseFloat(formData.amount) * 1e6));

      // Execute DCA swap
      setCurrentStep('approve');
      updateStepStatus('approve', 'in_progress');

      const result = await zerodevSmartWalletService.executeDCASwap(
        currentWalletId,
        swapAmount,
        formData.password,
      );
      ...
      if (!result.success) {
        throw new Error(result.error || 'DCA execution failed');
      }

      // Update step statuses based on the 3-step process
      updateStepStatus('approve', 'completed', result.transactionHash);
      
      setCurrentStep('swap');
      updateStepStatus('swap', 'in_progress');
      
      // Wait a bit then mark as completed
      setTimeout(() => {
        updateStepStatus('swap', 'completed', result.transactionHash);
        
        setCurrentStep('transfer');
        updateStepStatus('transfer', 'in_progress');
        
        setTimeout(() => {
          updateStepStatus('transfer', 'completed', result.transactionHash);
          setCurrentStep(null);
          
          toast.success(
            `🎉 DCA swap completed! ${(Number(swapAmount) / 1e6).toFixed(6)} USDC → SPX`,
            { duration: 6000 }
          );

          // Reset form
          setFormData({ amount: '10', password: formData.password });
      ...
      toast.error(
        error instanceof Error ? error.message : 'DCA execution failed',
      );
    } finally {
  ...
  const hasInsufficientBalance = Number.parseFloat(formData.amount) > usdcBalanceFormatted;
    ...
    <div className={`bg-gray-900 rounded-lg p-6 ${className}`}>
      ...
      {smartWalletAddress && (
        <div className="mb-6 p-4 bg-gray-800 rounded-lg">
          <h4 className="text-sm font-medium text-gray-300 mb-3">Smart Wallet</h4>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white">{usdcBalanceFormatted.toFixed(6)} USDC</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">SPX Balance:</span>
              <span className="text-white">{spxBalanceFormatted.toFixed(8)} SPX</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          {hasInsufficientBalance && (
            <p className="text-red-400 text-sm mt-1">
              Insufficient balance. You need {formData.amount} USDC but only have {usdcBalanceFormatted.toFixed(6)} USDC.
            </p>
          )}
        </div>
      ...
      <div className="mb-6 p-4 bg-purple-900/20 border border-purple-700 rounded-lg">
        ...
        <div className="space-y-2 text-sm text-purple-200">
          <div className="flex items-center gap-2">
            ...
            <span>Gas-free transactions with paymaster</span>
          </div>
          <div className="flex items-center gap-2">
            <Shield size={14} className="text-green-400" />
            <span>Agent-created keys for maximum security</span>
          </div>
          <div className="flex items-center gap-2">
            <Repeat size={14} className="text-blue-400" />
            <span>3-step process: Approve → Swap → Transfer</span>
          </div>
          <div className="flex items-center gap-2">
            <CheckCircle size={14} className="text-purple-400" />
            <span>KERNEL_V3_2 with chain abstraction support</span>
          </div>
        </div>
      </div>
