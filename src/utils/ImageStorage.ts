...
function findImageUrlOnPage(html: string, profileUrl: string): string | null {
    try {
        ...
        if (imageUrl && imageUrl.includes('profile_images')) {
            console.log(`[Debug] Found profile image URL via og:image on ${profileUrl}: ${imageUrl}`);
            return imageUrl;
        }
        imageUrl = $('meta[name="twitter:image"]').attr('content');
        if (imageUrl && imageUrl.includes('profile_images')) {
            console.log(`[Debug] Found profile image URL via twitter:image on ${profileUrl}: ${imageUrl}`);
            return imageUrl;
        }
        console.log(`[Debug] Meta tags did not yield profile image on ${profileUrl}.`);
        ...
        $('img').each((i, el) => {
             ...
             if (src && src.includes('pbs.twimg.com/profile_images/')) {
                 console.log(`[Debug] Found image via <img> src attribute on ${profileUrl}: ${src}`);
                 imgTagUrl = src;
        ...
        if (imgTagUrl) {
            return imgTagUrl; // Return the found src
             ...
             console.log(`[Debug] Could not find any <img> tag with src containing 'pbs.twimg.com/profile_images/' on ${profileUrl}.`);
        }
        // --- End Strategy 2 ---


        // If no strategies worked
        console.warn(`[Debug] Failed to extract profile image URL from HTML of ${profileUrl}`);
        ...
        console.error(`[Debug] Cheerio failed to parse HTML for ${profileUrl}:`, parseError);
        return null;
    }
}

// --- REVISED getCurrentTwitterImageUrl using Puppeteer ---
async function getCurrentTwitterImageUrl(profileUrl: string): Promise<string | null> {
    // Validate URL (lowercase check from before)
    const lowerCaseUrl = profileUrl ? profileUrl.toLowerCase() : '';
    if (!profileUrl || (!lowerCaseUrl.includes('twitter.com/') && !lowerCaseUrl.includes('x.com/'))) {
        console.log(`[Puppeteer Debug] Invalid or non-Twitter/X URL passed: ${profileUrl}`);
        return null;
    }
    ...
    try {
        ...
        browser = await puppeteer.launch({
             ...
             args: [ // Arguments to help avoid detection
                 '--no-sandbox',
        ...
        try {
            ...
            const imageUrl = await page.$eval(imageSelector, (img) => (img as HTMLImageElement).src);

            if (imageUrl && imageUrl.includes('profile_images')) {
                console.log(`[Puppeteer Success] Found image URL via selector ${imageSelector}: ${imageUrl}`);
                return imageUrl; // Found it!
            } else {
                 console.warn(`[Puppeteer Debug] Selector '${imageSelector}' found, but src is invalid: ${imageUrl}`);
                 return null;
            }

        } catch (selectorError) {
             console.warn(`[Puppeteer Debug] Could not find selector '${imageSelector}' on ${profileUrl}. Trying alternative methods or failing.`);
...
export async function downloadAndStoreProfileImages(profiles: any[]) {
    ...
    for (const profile of profiles) {
      ...
      console.log(`  Platform: ${profile.platform}, Profile URL: ${profile.profile_url}, Image URL (JSON): ${profile.image_url}`);
      ...
      try {
          ...
          if (!FORCE_UPDATE) {
              ...
              if (checkExisting) {
                   console.log(`  Skipping ${profile.username}, image already exists in cache (FORCE_UPDATE is false).`);
                   results.skipped++;
                   continue;
              }
          } else {
          ...
          console.log(`  Is X Platform: ${isXPlatform}, Has Profile URL: ${hasProfileUrl}`);


          // --- Decision Logic for Image URL Source ---
          if (isXPlatform && hasProfileUrl) {
              console.log(`  Platform is X with profile URL. Attempting live fetch via getCurrentTwitterImageUrl...`);
              ...
              if (!baseImageUrl) {
                  console.warn(`  Live fetch failed for X user ${profile.username}. Skipping image download for this profile.`);
              ...
              if (profile.image_url) {
                  console.log(`  Not an X profile OR missing profile_url. Using image_url from JSON: ${profile.image_url}`);
                  baseImageUrl = profile.image_url;
              } else {
                   console.warn(`  Cannot determine image URL for ${profile.username} (Not X/No profile_url, and no image_url in JSON). Skipping.`);
                   results.skipped++;
                   continue;
              }
          }
          ...
          try {
              response = await fetch(imageUrlToFetch, { // OK to use imageUrlToFetch here
                  headers: { // Use consistent headers
                       ...
                       'Referer': profile.profile_url || (isXPlatform ? 'https://x.com/' : undefined),
                  },
                  redirect: 'follow',
                  signal: AbortSignal.timeout(15000) // Timeout
              });
          } catch (error) {
              console.warn(`[Download] Failed fetch for ${profile.username} from ${imageUrlToFetch}. Error: ${error}`);
              results.failed++;
              continue;
          }
          ...
          if (!response.ok) {
               ...
               if (response.status === 404 && finalUrlIs400x400 && baseImageUrl !== imageUrlToFetch) {
          ...
          console.log(`[Download] Success for ${profile.username} (${contentType})`);

          const buffer = await response.arrayBuffer();
          if (buffer.byteLength === 0) {
              console.warn(`[Download] Received empty image buffer for ${profile.username} from ${imageUrlToFetch}, skipping...`);
              results.failed++;
              continue;
          }
          ...
          if (FORCE_UPDATE && existingData) { // Check the variable defined in the cache check block
                results.updated++;
                console.log(`[Store] Updated image for ${profile.username} in Redis.`);
           } else {
                results.success++;
                console.log(`[Store] Stored new image for ${profile.username} in Redis.`);
           }
        ...
        console.error(`[Overall Error] Failed processing profile ${profile.username}:`, error);
        results.failed++;
    }
  } // End for loop
