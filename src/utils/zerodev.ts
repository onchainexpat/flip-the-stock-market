...
export const createZeroDevSigner = async (privyWallet: any) => {
  ...
  if (isEmbeddedWallet) {
    console.log(
      '📱 Embedded wallet detected - relying on Privy configuration for Base network',
    );
  } else {
    // For external wallets, do normal network switching
    try {
      console.log(
        '🔄 Switching external wallet to Base network (Chain ID 8453)...',
      );
  ...
  const customAccount = {
    ...
    signMessage: async ({
      ...
      try {
        ...
        if (isEmbeddedWallet) {
          ...
          const embeddedMethods = [
            ...
            },
            {
              ...
              method: async () => {
                console.log(
                  '🔐 Trying raw provider without network validation...',
                );
          ...
          console.log(
            '❌ All embedded wallet methods failed, falling through to provider methods...',
          );
        }
        ...
        if (isEmbeddedWallet) {
          console.log(
            '📱 Embedded wallet - skipping network switch, using Privy configuration',
          );
        } else {
          try {
            console.log(
              '🔄 Ensuring Base network (Chain ID 8453) before signing...',
            );
            await provider.request({
              method: 'wallet_switchEthereumChain',
              params: [{ chainId: '0x2105' }], // 8453 in hex
        ...
        try {
          ...
          try {
            ...
            console.log(
              '✅ Message signed with eth_signTypedData_v4:',
              signature,
            );
            return signature as `0x${string}`;
...
export const getZeroDevConfig = (chain: Chain = base) => {
  ...
  if (
    NEXT_PUBLIC_ZERODEV_PROJECT_ID === '485df233-2a0d-4aee-b94a-b266be42ea55'
  ) {
    console.warn('⚠️ Using example project ID from .env.example');
    console.warn(
      '⚠️ Create a real ZeroDev project at https://dashboard.zerodev.app/',
    );
...
export const createEcdsaValidator = async (publicClient: any, signer: any) => {
...
export const createZeroDevKernelAccount = async (
  ...
  try {
    ...
    try {
      ...
      console.log(
        '🔧 Creating ECDSA validator with ZeroDev official approach...',
      );
      ...
      const combinations = [
        {
          ...
          name: 'ZeroDev v0.6 + KERNEL_V3_1',
        },
        {
          entryPoint: getEntryPoint('0.7'),
          kernelVersion: KERNEL_V3_1,
          name: 'ZeroDev v0.7 + KERNEL_V3_1',
        },
      ...
      if (!kernelAccount) {
        ...
        throw new Error(
          `Kernel account creation failed with all combinations: ${errors.join(', ')}`,
        );
      }
    }
...
export const createZeroDevKernelClient = async (
  ...
  try {
    ...
    try {
      ...
      console.log(
        '🔧 Creating kernel account client with separate transports...',
      );
      console.log('🔍 Bundler URL:', config.bundlerUrl);
      ...
      throw new Error(
        `Failed to create kernel client. Unified: ${unifiedError.message}, Separate: ${separateError.message}`,
      );
    }
  }
};
