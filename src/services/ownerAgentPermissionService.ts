...
export class OwnerAgentPermissionService {
  ...
  async authorizeAgentPermissions(
    ...
    try {
      console.log('✅ Owner authorizing agent permissions...');
      ...
      // Create call policies for DCA permissions
      const callPolicies = [
        // Policy 1: Approve USDC for OpenOcean router
        toCallPolicy({
          target: TOKENS.USDC,
          ...
          rules: [
            {
              ...
              param: OPENOCEAN_ROUTER,
            },
          ],
        }),
        // Policy 2: Execute swaps on OpenOcean router
        toCallPolicy({
          target: OPENOCEAN_ROUTER,
          selector: '0x90411a32', // swap function selector
          rules: [],
        }),
        // Policy 3: Transfer SPX tokens to user's wallet only
        toCallPolicy({
        ...
        // Policy 4: Transfer USDC back to user (for sweeping)
        toCallPolicy({
          target: TOKENS.USDC,
          selector: '0xa9059cbb', // transfer(address,uint256)
          rules: [
      ...
      const permissionValidator = toPermissionValidator(this.publicClient, {
...
export const ownerAgentPermissionService = new OwnerAgentPermissionService();
