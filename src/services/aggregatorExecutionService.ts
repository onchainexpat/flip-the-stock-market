...
export class AggregatorExecutionService {
  ...
  async getBestExecutableSwap(
    ...
    if (availableAggregators.includes('OpenOcean')) {
      swapPromises.push(this.getSwapDataWithRetry('OpenOcean', sellToken, buyToken, sellAmount, userAddress));
    }
    if (availableAggregators.includes('1inch')) {
      swapPromises.push(this.getSwapDataWithRetry('1inch', sellToken, buyToken, sellAmount, userAddress));
    }
    if (availableAggregators.includes('Paraswap')) {
      swapPromises.push(this.getSwapDataWithRetry('Paraswap', sellToken, buyToken, sellAmount, userAddress));
    }
    ...
    if (swaps.length === 0) {
      ...
      const fallbackSwap = await this.getEmergencyFallbackSwap(sellToken, buyToken, sellAmount, userAddress);
      if (fallbackSwap) {
        swaps.push(fallbackSwap);
        console.log('🚨 Using emergency fallback swap data');
      } else {
        throw new Error('All aggregators failed and no fallback swap data available');
      }
    }
    ...
    const alternativeSwaps = swaps.filter(swap => swap.aggregator !== bestSwap.aggregator);
    const nextBestSwap = alternativeSwaps.length > 0 
    ...
    const savingsAmount = BigInt(bestSwap.buyAmount) - BigInt(nextBestSwap.buyAmount);
    const savingsPercentage = nextBestSwap.buyAmount !== '0' 
      ? Number(savingsAmount * 10000n / BigInt(nextBestSwap.buyAmount)) / 100 
      : 0;

    console.log(`✅ Best swap: ${bestSwap.aggregator} - ${bestSwap.buyAmount} tokens`);
    console.log(`💰 Savings: ${savingsAmount.toString()} tokens (${savingsPercentage.toFixed(2)}%)`);
  ...
  private async getSwapDataWithRetry(
    ...
    try {
      switch (aggregator) {
        case 'OpenOcean':
          return await this.getOpenOceanSwapData(sellToken, buyToken, sellAmount, userAddress);
        case '1inch':
          return await this.get1inchSwapData(sellToken, buyToken, sellAmount, userAddress);
        case 'Paraswap':
          return await this.getParaswapSwapData(sellToken, buyToken, sellAmount, userAddress);
        default:
          throw new Error(`Unknown aggregator: ${aggregator}`);
      }
    } catch (error) {
      if (retryCount < this.MAX_RETRIES) {
        console.log(`🔄 Retrying ${aggregator} swap data (attempt ${retryCount + 1}/${this.MAX_RETRIES})`);
        await this.delay(2000 * (retryCount + 1)); // Longer delay for swap data
        return this.getSwapDataWithRetry(aggregator, sellToken, buyToken, sellAmount, userAddress, retryCount + 1);
      }
      throw error;
    }
  }
  ...
  private async getOpenOceanSwapData(
    ...
    try {
      ...
      return this.createErrorSwap('OpenOcean', sellToken, buyToken, sellAmount, error);
    }
  }
  ...
  private async get1inchSwapData(
    ...
    try {
      ...
      if (!data.tx.to || data.tx.to === '0x0000000000000000000000000000000000000000') {
      ...
      return this.createErrorSwap('1inch', sellToken, buyToken, sellAmount, error);
    }
  }
  ...
  private async getParaswapSwapData(
    ...
    try {
      ...
      if (!priceData.priceRoute.destAmount || priceData.priceRoute.destAmount === '0') {
      ...
      const minReceived = BigInt(priceData.priceRoute.destAmount) * 985n / 1000n;
      ...
      return this.createErrorSwap('Paraswap', sellToken, buyToken, sellAmount, error);
    }
  }
   ...
   */
  async getBestSPXSwap(usdcAmount: string, userAddress: Address): Promise<SwapExecutionResult> {
   ...
   */
  validateSwapData(swapData: ExecutableSwapData): { valid: boolean; error?: string } {
    if (!swapData.success) {
      return { valid: false, error: swapData.error || 'Swap data indicates failure' };
    }

    if (!swapData.to || swapData.to === '0x0000000000000000000000000000000000000000') {
    ...
    if (priceImpact > 5) { // More than 5% price impact
  ...
  private recordFailure(aggregator: string): void {
    ...
    if (breaker.failures >= this.CIRCUIT_BREAKER_THRESHOLD) {
      breaker.isOpen = true;
      console.warn(`🚨 Execution circuit breaker OPENED for ${aggregator} (${breaker.failures} failures)`);
    }
  ...
  private async getEmergencyFallbackSwap(
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Emergency API failed: ${response.status}`);
