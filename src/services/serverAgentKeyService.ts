...
function getEncryptionKey(): string {
  ...
  if (!key || key.length < 32) {
    throw new Error('AGENT_KEY_ENCRYPTION_SECRET must be set and at least 32 characters long');
  }
  return key;
}
...
const REDIS_KEYS = {
  ...
  AGENT_KEY_BY_WALLET: (smartWalletAddress: string) => `agent:wallet:${smartWalletAddress}:key`,
};
...
export class ServerAgentKeyService {
  ...
  async storeSessionKey(
    ...
    try {
      ...
      const encryptedPrivateKey = await this.encryptPrivateKey(sessionPrivateKey);
      ...
      // Store in Redis
      await getRedisClient().set(REDIS_KEYS.AGENT_KEY(keyId), JSON.stringify(agentKey));
      await getRedisClient().sadd(REDIS_KEYS.USER_AGENT_KEYS(userAddress), keyId);
      await getRedisClient().set(REDIS_KEYS.AGENT_KEY_BY_WALLET(smartWalletAddress), keyId);
  ...
  async getPrivateKey(keyId: string): Promise<Hex | null> {
    const agentKeyData = await getRedisClient().get(REDIS_KEYS.AGENT_KEY(keyId));
   ...
   */
  async getAgentKeyByWallet(smartWalletAddress: Address): Promise<ServerAgentKey | null> {
    const keyId = await getRedisClient().get(REDIS_KEYS.AGENT_KEY_BY_WALLET(smartWalletAddress));
    if (!keyId) return null;
    
    const agentKeyData = await getRedisClient().get(REDIS_KEYS.AGENT_KEY(keyId as string));
    if (!agentKeyData) return null;
   ...
   */
  async updateSmartWalletAddress(keyId: string, smartWalletAddress: Address): Promise<boolean> {
    const agentKeyData = await getRedisClient().get(REDIS_KEYS.AGENT_KEY(keyId));
  ...
  async getUserAgentKeys(userAddress: Address): Promise<ServerAgentKey[]> {
    const keyIds = await getRedisClient().smembers(REDIS_KEYS.USER_AGENT_KEYS(userAddress));
   ...
   */
  async updateAgentKey(keyId: string, updates: Partial<ServerAgentKey>): Promise<boolean> {
    const agentKeyData = await getRedisClient().get(REDIS_KEYS.AGENT_KEY(keyId));
    if (!agentKeyData) return false;
    ...
    await getRedisClient().set(REDIS_KEYS.AGENT_KEY(keyId), JSON.stringify(updatedKey));
    
    // Update smart wallet index if needed
    if (updates.smartWalletAddress && updates.smartWalletAddress !== agentKey.smartWalletAddress) {
      await getRedisClient().set(REDIS_KEYS.AGENT_KEY_BY_WALLET(updates.smartWalletAddress), keyId);
    }
   ...
   */
  async cleanupOldKeys(daysUnused: number = 90): Promise<number> {
