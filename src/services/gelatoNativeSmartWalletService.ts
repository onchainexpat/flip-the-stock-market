...
export class GelatoNativeSmartWalletService {
  ...
  static async executeDCASwap(
    ...
    try {
      ...
      console.log(
        '   Agent key owner (should be undefined):',
        agentKey.ownerAddress,
      );
      ...
      if (!this.sponsorApiKey || typeof this.sponsorApiKey !== 'string') {
        throw new Error(
          `Invalid sponsor API key: ${typeof this.sponsorApiKey} - ${this.sponsorApiKey}`,
        );
      }
      ...
      console.log(
        '💰 Automation wallet USDC balance:',
        (Number(automationBalance) / 1e6).toFixed(6),
        'USDC',
      );
      ...
      if (automationBalance < amountUSDC) {
        ...
        const { SponsoredFundingService } = await import(
          './sponsoredFundingService'
        );
        ...
        if (sponsorStatus.available) {
          ...
          console.log(
            '   Sponsor balance:',
            sponsorStatus.sponsorBalance,
            'USDC',
          );

          // Get optimal funding amount (buffer for future executions)
          const fundingAmount =
            SponsoredFundingService.getMinimumFundingAmount(needed);
          console.log(
            '   Funding amount:',
            (Number(fundingAmount) / 1e6).toFixed(6),
            'USDC',
          );

          const fundingResult =
            await SponsoredFundingService.fundAutomationWallet(
          ...
          if (fundingResult.success) {
            ...
            console.log(
              '💰 New automation wallet balance:',
              (Number(newBalance) / 1e6).toFixed(6),
              'USDC',
            );
      ...
      if (taskStatus.executed && taskStatus.txHash) {
        ...
        console.log(
          '   🎯 SPX tokens should now be in automation wallet:',
          automationAccount.address,
        );
        console.log(
          '   💡 Note: Tokens are in automation wallet for future DCA executions',
        );
        return {
          success: true,
        ...
        console.log(
          '❌ Swap task execution failed or timed out:',
          taskStatus.status,
        );
        return {
          success: false,
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (statusResponse.ok) {
          ...
          console.log(
            `📊 Task ${taskId} status (attempt ${attempt}):`,
          ...
          if (
            ...
            console.log(
              `✅ Task executed successfully: ${taskStatus.transactionHash}`,
            );
            return {
          ...
          console.log(
            `⚠️ Failed to check task status (attempt ${attempt}): ${statusResponse.status}`,
          );
          if (attempt < maxAttempts) {
        ...
        console.log(
          `⚠️ Error checking task status (attempt ${attempt}):`,
          error,
        );
        if (attempt < maxAttempts) {
          await new Promise((resolve) => setTimeout(resolve, 5000));
