  ...
  static async executeOneClickDCA(
    ...
    try {
      ...
      try {
        ...
        const { ClientGaslessSessionKeyService } = await import(
          './clientGaslessSessionKeyService'
        );
        ...
        const sessionKeyResult =
          await ClientGaslessSessionKeyService.createSimpleClientGaslessSessionKey(
            wallet,
        ...
        if (sessionKeyResult.success) {
          ...
          console.error(
            '❌ Gasless session key creation failed:',
            sessionKeyResult.error,
          );
      ...
      console.log(
        "✅ Gas sponsorship enabled - smart wallet doesn't need ETH funding",
      );
...
export function useOneClickDCA() {
  ...
  const executeOneClickDCA = async (
    params: OneClickDCAParams,
  ): Promise<OneClickDCAResult> => {
