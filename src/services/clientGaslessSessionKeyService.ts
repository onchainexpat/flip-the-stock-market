...
import { createBasePublicClient, createZeroDevSigner } from '../utils/zerodev';
...
export class ClientGaslessSessionKeyService {
  ...
  static async createClientSideGaslessSessionKey(
    ...
    try {
      ...
      if (
        permissionAccount.address.toLowerCase() !==
      ...
      const sessionKeyApproval = await serializePermissionAccount(
        permissionAccount,
        sessionPrivateKey, // Include session private key for proper deserialization
      );
      ...
      const storeResponse = await fetch(
        `${window.location.origin}/api/store-client-session-key`,
        {
          method: 'POST',
          headers: {
      ...
      console.error(
        '❌ Client-side gasless session key creation failed:',
        error,
      );
      return {
        success: false,
