...
async function checkOrderStatus() {
  ...
  try {
    ...
    console.log('  - Next Execution:', order.nextExecutionTime ? new Date(order.nextExecutionTime).toLocaleString() : 'N/A')
    console.log('  - Last Execution:', order.lastExecutionTime ? new Date(order.lastExecutionTime).toLocaleString() : 'Never')
    ...
    // 4. Summary
    console.log('\n📌 Summary:')
    console.log(`  - Order ${orderId} is currently ${order.status.toUpperCase()}`)
    console.log(`  - ${order.executionsCount} out of ${order.totalIntervals} executions completed`)
    
    if (order.executionsCount > 0) {
      const successRate = (order.executionsCount / (order.executionsCount + (order.error ? 1 : 0))) * 100
    ...
    if (order.status === 'paused' && order.error) {
      if (order.error.includes('rate limit')) {
        ...
        console.log('  - Consider adjusting execution interval to avoid rate limits')
