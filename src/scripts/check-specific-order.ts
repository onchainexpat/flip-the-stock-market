...
async function checkSpecificOrder() {
  ...
  try {
    ...
    console.log(`  Next Execution: ${orderData.nextExecution ? new Date(parseInt(orderData.nextExecution)).toISOString() : 'N/A'}`);
    console.log(`  Last Execution: ${orderData.lastExecution ? new Date(parseInt(orderData.lastExecution)).toISOString() : 'Never'}`);
    console.log(`  Creation Time: ${orderData.createdAt ? new Date(parseInt(orderData.createdAt)).toISOString() : 'N/A'}`);
    ...
    const executionLogs = await redis.lrange(`dca:order:${orderId}:executions`, 0, -1);
    console.log('\n📋 Execution Logs:');
    if (executionLogs && executionLogs.length > 0) {
      executionLogs.forEach((log: any, index: number) => {
        ...
        console.log(`  [${index + 1}] ${new Date(logData.timestamp).toISOString()}`);
    ...
    console.log(`  Amount Per Execution: ${orderData.amountPerExecution} ${orderData.inputToken}`);
