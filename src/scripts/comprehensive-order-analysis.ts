...
async function comprehensiveOrderAnalysis() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        '└─ Session Created:',
        new Date(sessionKeyInfo.createdAt).toLocaleString(),
      );
    } catch (e) {
    ...
    console.log(
      '├─ Next Execution Time:',
      new Date(orderData.nextExecutionAt).toLocaleString(),
    );
    console.log(
      '├─ Expires At:',
    ...
    console.log(
      '└─ Transaction Hashes:',
      orderData.executionTxHashes.length > 0
        ? orderData.executionTxHashes
        : 'None',
    );
    ...
    if (sessionKeyInfo?.agentKeyId) {
      console.log('\n🔍 AGENT KEY CHECK');
      const agentKeyData = await redis.get(
        `agent_key:${sessionKeyInfo.agentKeyId}`,
      );
      if (agentKeyData) {
        console.log('✅ Agent key found:', sessionKeyInfo.agentKeyId);
        const agentInfo =
          typeof agentKeyData === 'object'
            ? agentKeyData
            : JSON.parse(agentKeyData.toString());
        console.log('├─ Status:', agentInfo.status || 'active');
        console.log('├─ Smart Wallet:', agentInfo.smartWalletAddress);
        console.log(
          '└─ Created:',
          new Date(agentInfo.createdAt).toLocaleString(),
        );
      } else {
    ...
    console.log(
      `  ├─ ${orderData.executionsCount}/${orderData.totalExecutions} executions completed`,
    );
    console.log(
      `  ├─ ${orderData.executedAmount}/${orderData.totalAmount} amount executed`,
    );

    const timeSinceCreation = Date.now() - orderData.createdAt;
    const timeSinceUpdate = Date.now() - orderData.updatedAt;
    console.log(
      `  ├─ Created ${Math.round(timeSinceCreation / (1000 * 60))} minutes ago`,
    );
    console.log(
      `  └─ Last updated ${Math.round(timeSinceUpdate / (1000 * 60))} minutes ago`,
    );
    ...
    if (orderData.status === 'paused' && orderData.executionsCount === 0) {
      ...
      console.log(
        '  ❌ This indicates an issue during the first execution attempt',
      );

      if (orderData.nextExecutionAt < Date.now()) {
        console.log(
          '  ⏰ Next execution time has passed - order should have executed',
        );
      }

      if (sessionKeyInfo?.provider === 'gelato-native') {
        console.log(
          '  🤖 Uses Gelato Native provider - check Gelato dashboard for task status',
        );
      }
    }
    ...
    console.log(
      '  3. Check if rate limits or API issues occurred during execution',
    );
