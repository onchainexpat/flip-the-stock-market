    ...
    if (data && typeof data === 'object') {
      ...
      if (order.status === 'paused') {
        ...
        if (order.executionsCount === 0) {
          ...
          console.log('  - This suggests the issue occurred during first execution attempt')
        } else {
          console.log(`  - ${order.executionsCount} successful execution(s) before pausing`)
        }
        
        if (order.error) {
          console.log('  - Error details available - likely auto-paused due to error')
        }
      }
    }
