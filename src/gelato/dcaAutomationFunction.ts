...
class GelatoMultiAggregatorService {
  ...
  async getBestSwapQuote(
    ...
    const validQuotes = quotes
      .map((result, index) => {
        ...
        console.log(`Aggregator ${index} failed:`, result.status === 'rejected' ? result.reason : 'Invalid response');
        return null;
      })
      .filter(Boolean);
    ...
    const bestQuote = validQuotes.reduce((best, current) => {
      return BigInt(current.buyAmount) > BigInt(best.buyAmount) ? current : best;
    });

    console.log(`Best quote from ${bestQuote.bestAggregator}: ${bestQuote.buyAmount} tokens`);
    return bestQuote;
  }

  private async getOpenOceanQuote(sellToken: string, buyToken: string, sellAmount: string, userAddress: string) {
    try {
      ...
      const response = await fetch(`https://open-api.openocean.finance/v4/${this.BASE_CHAIN_ID}/swap_quote?${params}`, {
        method: 'GET',
      ...
      if (!data.data || !data.outAmount) throw new Error('Invalid OpenOcean response');
  ...
  private async get1inchQuote(sellToken: string, buyToken: string, sellAmount: string, userAddress: string) {
    try {
      const params = new URLSearchParams({
      ...
      const response = await fetch(`https://api.1inch.dev/swap/v6.0/${this.BASE_CHAIN_ID}/swap?${params}`, {
        method: 'GET',
      ...
      const data = await response.json();
      if (!data.tx || !data.toTokenAmount) throw new Error('Invalid 1inch response');
  ...
  private async getParaswapQuote(sellToken: string, buyToken: string, sellAmount: string, userAddress: string) {
    try {
      ...
      if (!priceResponse.ok) throw new Error(`Paraswap price error: ${priceResponse.status}`);
      
      const priceData = await priceResponse.json();
      if (!priceData.priceRoute) throw new Error('No Paraswap price route');

      // Get transaction data
      const txResponse = await fetch('https://apiv5.paraswap.io/transactions/8453', {
        method: 'POST',
      ...
      if (!txResponse.ok) throw new Error(`Paraswap tx error: ${txResponse.status}`);
  ...
  try {
    ...
    for (const order of ordersToProcess) {
      try {
        ...
        console.log(`✅ Order ${order.id}: Best rate from ${swapQuote.bestAggregator}`);
    ...
    console.log(`📈 Processed ${ordersToProcess.length} orders, ${successfulOrders} successful`);
