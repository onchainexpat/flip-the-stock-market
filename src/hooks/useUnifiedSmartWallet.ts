...
interface SmartWalletState {
  ...
  walletType: 'embedded_privy' | 'external_wallet' | 'coinbase_smart' | 'zerodev_smart' | null;
...
export function useUnifiedSmartWallet() {
  ...
  const detectWalletConfiguration = useCallback(async () => {
    ...
    try {
      ...
      if (externalWallet || (isConnected && wagmiAddress)) {
        ...
        console.log('🔗 External wallet detected, configuring ZeroDev smart wallet...');
        console.log('🔍 Wallet source:', externalWallet ? 'Privy external' : 'Wagmi direct');
        ...
        try {
          ...
          const code = await publicClient.getBytecode({ address: kernelAccount.address });
      ...
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: error instanceof Error ? error.message : 'Wallet detection failed',
      }));
    }
  }, [ready, authenticated, externalWallet, embeddedWallet, isConnected, wagmiAddress, user]);
  ...
  const deploySmartWallet = useCallback(async (): Promise<string | null> => {
    if (!smartWalletAccount || state.walletType !== 'zerodev_smart') {
      throw new Error('Smart wallet deployment not available for this wallet type');
    }
    ...
    try {
      ...
      const kernelClient = await createZeroDevKernelClient(smartWalletAccount, base);
      setSmartWalletClient(kernelClient);
      ...
      setState(prev => ({ ...prev, needsDeployment: false, isLoading: false }));
  ...
  const generateSessionKey = useCallback(async (
    ...
    try {
      if (state.walletType === 'coinbase_smart') {
        ...
        const sessionData = await coinbaseSmartWalletService.generateSessionKey(
          state.address,
          permissions
        );
  ...
  const sendTransaction = useCallback(async (transaction: {
    ...
    try {
      if (state.walletType === 'zerodev_smart' && smartWalletClient) {
        ...
        console.log('   💰 Value (ETH):', transaction.value ? (Number(transaction.value) / 1e18).toFixed(6) : '0');
        console.log('   📊 Gas Limit:', transaction.gas || 'Auto-estimated');
        console.log('   🎯 Transaction Type: Smart Wallet UserOperation (ERC-4337)');
        console.log('   🔐 Signature Purpose: Authorize smart wallet to execute this transaction');
        console.log('   ⛽ Gas Sponsorship: ZeroDev paymaster (transaction is FREE)');
        
        if (transaction.data && transaction.data !== '0x') {
          console.log('   📝 Transaction Data Present: Yes (contract interaction)');
          console.log('   📝 Data Length:', transaction.data.length, 'characters');
        ...
        console.log('   ✅ This signature authorizes your smart wallet to execute ONE specific transaction');
        console.log('   ✅ The signature cannot be reused for other transactions');
        console.log('   ✅ Gas fees are sponsored - you pay nothing');
        console.log('   ✅ You can revoke smart wallet permissions at any time');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        ...
        console.log('   ⛽ Gas Sponsorship Available:', state.hasGasSponsorship);
        ...
        console.log('⛽ Letting ZeroDev estimate gas and handle paymaster...');
        ...
        try {
          ...
          if (error.message?.includes('insufficient funds')) {
            console.error('💰 INSUFFICIENT FUNDS ERROR - Check smart wallet balance');
          } else if (error.message?.includes('allowance')) {
            console.error('🔐 ALLOWANCE ERROR - USDC approval might be needed');
          } else if (error.message?.includes('simulation')) {
            console.error('⚠️ SIMULATION ERROR - Transaction would fail on-chain');
            console.error('💡 Common causes: insufficient balance, missing approval, or contract error');
          }
        ...
        throw new Error('Coinbase smart wallet transactions not yet implemented in this hook');
      }
  ...
  }, [state.address, state.hasGasSponsorship, state.walletType, smartWalletClient, smartWalletAccount]);

  // Send batch of transactions (for DCA setup, swaps, cleanup)
  const sendBatchTransactions = useCallback(async (transactions: Array<{
    to: Address;
    value?: bigint;
    data?: string;
    description?: string;
    ...
    try {
      ...
      const externalWalletTxs = transactions.filter(tx => tx.executeFrom !== 'smart_wallet');
      const smartWalletTxs = transactions.filter(tx => tx.executeFrom === 'smart_wallet');

      // Execute external wallet transactions first (e.g., USDC transfer to smart wallet)
      if (externalWalletTxs.length > 0 && activeWallet) {
        console.log('🔗 Executing', externalWalletTxs.length, 'transactions from external wallet...');
      ...
      if (smartWalletTxs.length > 0 && smartWalletClient && smartWalletAccount) {
        console.log('🤖 Executing', smartWalletTxs.length, 'transactions from smart wallet...');
  ...
  return {
    ...
    // Utility
    isExternalWallet: state.walletType === 'external_wallet' || state.walletType === 'zerodev_smart',
    isPrivyWallet: state.walletType === 'embedded_privy' || state.walletType === 'coinbase_smart',
