...
import DCAQuickSetup from '../components/DCA/DCAQuickSetup';
...
const formatPrice = (
  ...
  const effectiveMaxDecimals =
    value > 0 && value < 0.01 ? Math.max(maxDecimals, 4) : maxDecimals;
...
const PriceComparison = React.forwardRef<HTMLDivElement, PriceComparisonProps>(
  (
    ...
    const multiplier =
      snpMarketCap && spxMarketCap && spxMarketCap > 0
        ? snpMarketCap / spxMarketCap
        : null;

    const priceAtSnPMC =
      snpMarketCap && spxMarketCap && spxPrice && spxMarketCap > 0
        ? (snpMarketCap / spxMarketCap) * spxPrice
        : null;

    // Calculate doublings needed for the flippening
    const doublingsNeeded =
    ...
    return (
      ...
      >
        <div
          className={`flex ${isForTwitter ? 'flex-row' : 'flex-col md:flex-row'} justify-between items-center md:items-start mb-4 w-full gap-6 md:gap-0`}
        >
          {/* SPX6900 Side */}
          <div
            className={`flex-1 text-white flex flex-col ${isForTwitter ? 'items-center text-center' : 'items-center md:items-start'} w-full md:w-auto`}
          >
            ...
            <div className="flex items-baseline gap-2 mb-2">
              <span className="text-2xl md:text-3xl font-bold">
                {formatPrice(spxPrice, 4, 4)}
              </span>
              <span
                className={`text-base md:text-lg ${spxChangeFormatted.color}`}
              >
                {spxChangeFormatted.text}
              </span>
            </div>
            <div className="text-sm md:text-base text-gray-300">
              Market Cap:{' '}
              <span className="text-green-400 font-medium">
                {formatMarketCap(spxMarketCap)}
              </span>
            </div>
          </div>
          ...
          <div
            className={`${isForTwitter ? 'w-px h-24' : 'w-full h-px md:w-px md:h-24'} bg-gray-800 md:mx-12 self-center`}
          ></div>
        ...
        {showExtraSection && (
          <div
            className="mt-3 pt-3 text-center text-white"
            data-market-cap-section={true}
          >
            <h3 className="text-base md:text-lg font-semibold mb-3">
              <span className="gold-text">SPX6900</span> WITH THE MARKET CAP OF{' '}
              <span className="gold-text">S&P500</span>
            </h3>
            <div className="flex items-baseline justify-center gap-2 mb-2">
              {' '}
              {/* Added mb-2 */}
              <span className="text-xl md:text-2xl font-bold">
                {formatPrice(priceAtSnPMC)}
              </span>
              {multiplier !== null && (
...
const PriceComparisonForImage = () => (
  ...
  >
    ...
    <div className="flex-1 text-white flex flex-col items-start">
      <div className="flex items-center gap-2 mb-3">
        <img src="/spx500-logo-circle.png" alt="S&P 500" className="w-7 h-7" />
    ...
    <div className="flex flex-col sm:flex-row w-full justify-center items-center gap-6 sm:gap-0 p-4 sm:p-6 bg-[#131827] rounded-lg max-w-[600px] mx-auto">
      ...
      <div className="flex-1 text-white flex flex-col items-center sm:items-start w-full sm:w-auto">
        ...
        <div className="flex items-baseline gap-2 mb-2">
          ...
          <span className={`text-sm sm:text-lg ${spxChangeFormatted.color}`}>
            {spxChangeFormatted.text}
          </span>
        </div>
      ...
      <div className="flex-1 text-white flex flex-col items-center sm:items-start w-full sm:w-auto">
        ...
        <div className="flex items-baseline gap-2 mb-2">
          ...
          <span className={`text-sm sm:text-lg ${snpChangeFormatted.color}`}>
            {snpChangeFormatted.text}
          </span>
        </div>
        <div className="text-sm sm:text-base text-gray-300">
...
export default function Page() {
  ...
  const [openDropdown, setOpenDropdown] = useState<
    'sponsoredBuys' | 'howToBuyVideo' | null
  >(null);
  ...
  const [purchasedTokenLogoUrl, setPurchasedTokenLogoUrl] = useState<
    string | null
  >(null); // State for the dynamic logo URL
  ...
  useEffect(() => {
    if (purchasedTokenLogoUrl) {
      ...
      img.onload = () => {
        console.log(
          `Confetti image loaded successfully: ${purchasedTokenLogoUrl}`,
        );
        confettiImage.current = img;
      ...
      img.onerror = (error) => {
        console.error(
          `Failed to load confetti image: ${purchasedTokenLogoUrl}`,
          error,
        );
    ...
    (lifecycleStatus: LifecycleStatus) => {
      ...
      if (statusName === 'success' && statusData?.tokenTo) {
        ...
        if (statusData.tokenTo.image) {
          ...
          console.log(
            'Image not directly in statusData.tokenTo, searching swappableTokens...',
          );
          confirmedToToken =
            swappableTokens.find(
              (t) =>
                t.address.toLowerCase() ===
                  statusData.tokenTo.address?.toLowerCase() &&
                t.chainId === statusData.tokenTo.chainId,
            ) ?? null;
          if (confirmedToToken) {
            console.log(
              'Found matching token in swappableTokens:',
              confirmedToToken,
            );
          } else {
            console.error(
              'Could not find matching token in swappableTokens for:',
              statusData.tokenTo,
            );
          }
        }
        ...
        if (confirmedToToken?.image) {
          ...
          console.error(
            'Could not determine confirmedToToken or its image URL for confetti.',
          );
        }
  ...
  useEffect(() => {
    const fetchParseHubProjects = async () => {
      ...
      try {
          ...
          projectTokens.map(async (project) => {
            try {
              const response = await fetch(
                `/api/parsehub?projectToken=${project.token}`,
              );
              if (!response.ok)
                throw new Error(`Failed to fetch ${project.title} data`);
              const data = await response.json();
        ...
        console.log(
          'Combined ParseHub data (excluding investing.com):',
          combinedData,
        );
  ...
  const handleCoinbaseWalletClick = useCallback((event: React.MouseEvent) => {
    ...
    // With Privy + ZeroDev, authentication is handled by ProfileDropdown
    console.log('Authentication handled by ProfileDropdown component');
  }, []);
  ...
  const generatePriceImage = async () => {
    ...
    if (!spxPrice || !spxMarketCap || !snpData || !parseHubData) {
      // Check snpData
      console.error('Data not yet available for image generation:', {
        spxPrice,
        spxMarketCap,
        snpData,
        parseHubData, // Use snpData
      });
      return null;
    }
    ...
    try {
      ...
      console.log('Data for image generation:', {
        spxPrice,
        spx24hChange,
        spxMarketCap,
        snpPrice,
      ...
      const mainFlexContainer = element.querySelector(
        '[data-price-comparison] > div',
      );
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          ...
          const capturedContainer = clonedDoc.body.querySelector(
            '[data-twitter-image="true"]',
          );

          if (capturedContainer instanceof HTMLElement) {
            console.log(
              'Found captured container [data-twitter-image="true"] within cloned body.',
            );

            // Now find the target element '[data-price-comparison]' WITHIN the captured container
            const clonedElement = capturedContainer.querySelector(
              '[data-price-comparison]',
            );

            if (clonedElement instanceof HTMLElement) {
              console.log(
                'Found target element [data-price-comparison] within captured container.',
              );
              // --- Re-apply styles within the clone ---
              const mainFlex = clonedElement.querySelector(
              ...
              // Style SPX side
              const spxSideClone = clonedElement.querySelector(
                ':scope > div:first-child > div:nth-child(1)',
              );
              if (spxSideClone instanceof HTMLElement) {
                ...
                spxSideClone.style.setProperty(
                  'text-align',
                  'center',
                  'important',
                );
                ...
                const spxTitleContainer =
                  spxSideClone.querySelector(':scope > div');
                if (spxTitleContainer instanceof HTMLElement) {
                  spxTitleContainer.style.setProperty(
                    'display',
                    'inline-flex',
                    'important',
                  );
              ...
              // Style SNP side
              const snpSideClone = clonedElement.querySelector(
                ':scope > div:first-child > div:nth-child(3)',
              );
              if (snpSideClone instanceof HTMLElement) {
                ...
                snpSideClone.style.setProperty(
                  'text-align',
                  'center',
                  'important',
                );
                ...
                const snpTitleContainer =
                  snpSideClone.querySelector(':scope > div');
                if (snpTitleContainer instanceof HTMLElement) {
                  snpTitleContainer.style.setProperty(
                    'display',
                    'inline-flex',
                    'important',
                  );
              ...
              console.error(
                "Target element '[data-price-comparison]' NOT found within captured container.",
                {
            ...
            console.error(
              'Captured container \'[data-twitter-image="true"]\' NOT found within cloned document body.',
              {
      ...
      return new Promise<Blob>((resolve, reject) => {
        try {
          // Add try...catch inside the promise executor
          canvas.toBlob(
            (blob) => {
              if (blob) {
                ...
                console.error(
                  'Blob creation failed: canvas.toBlob returned null',
                );
                reject(
                  new Error(
                    'Blob creation failed: canvas.toBlob returned null',
  ...
  const isDataReadyForShare =
    spxPrice !== null &&
    spxMarketCap !== null &&
    snpData !== null &&
    isParseHubDataLoaded; // Use snpData

  const handleShareToX = useCallback(async () => {
    setIsGeneratingImage(true);

    if (!isDataReadyForShare) {
      // Uses updated check
  ...
  useEffect(() => {
    const replaceBalanceText = () => {
      ...
      const balanceElements = document.querySelectorAll(
        '.swap-token-balance span:first-child',
      );
          ...
          <section className="templateSection flex w-full flex-col items-center justify-center gap-4 rounded-xl bg-transparent px-2 py-4">
              ...
              <span className="flex items-center gap-2 text-base font-medium">
                {isDataReadyForShare ? (
                  isGeneratingImage ? (
                    ...
                    <>
                      Post to{' '}
                      <img src="/x-logo.svg" alt="X Logo" className="w-5 h-5" />
                    </>
            ...
            <div className="w-full p-4 bg-[#1B2236]/40 backdrop-blur-md rounded-xl mt-4">
              <h2 className="text-2xl font-bold text-white text-center mb-4">
                <span className="gold-text">SPX6900</span> WITH THE MARKET CAP
                OF <span className="gold-text">S&P500</span>
              </h2>
              {/* Check for spxPrice, spxMarketCap and parseHubData.slickchart */}
              {isParseHubDataLoaded &&
                  ...
                  <div className="text-xl font-bold text-white text-center mb-4">
                    ...
                    <span className="text-green-400 ml-2">
                      ({(() => {
                        const snpMC = parseSnpMarketCap(parseHubData);
                        if (snpMC === null) return 'N/A';
                        return (snpMC / spxMarketCap).toLocaleString(
                          undefined,
                          {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                    ...
                    <div className="flex w-full max-w-[400px]">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const element =
                            document.getElementById('calculatedValue');
                          if (element) {
            ...
            {/* Trading Section - Side-by-side layout when authenticated */}
              ...
              {address ? (
                <div className="w-full max-w-6xl mx-auto">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {/* Manual Trading - Buy Now */}
                    <div className="flex flex-col">
                      <h2 className="text-2xl font-bold text-white mb-4 text-center">
                        Buy SPX6900 Now
                      </h2>
                      <p className="text-gray-300 text-center mb-4">
                        Trade instantly with the best price execution
                      </p>
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="text-gray-300 text-center mb-4">
                        Dollar cost average with automated purchases
                      </p>
                  ...
                  <div className="w-full bg-[#1B2236] bg-opacity-70 backdrop-blur-md rounded-xl shadow-md px-8 py-8 mb-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full mb-6">
                      {[
                        {
                          name: 'Bybit',
                          url: 'https://www.bybit.com/trade/spot/SPX/USDT/',
                          icon: '/bybit.svg',
                        },
                        {
                          name: 'Kraken',
                          url: 'https://pro.kraken.com/app/trade/SPX-USD',
                          icon: '/kraken.svg',
                        },
                        {
                          name: 'KuCoin',
                          url: 'https://www.kucoin.com/trade/SPX-USDT/',
                          icon: '/kukoin.svg',
                        },
                      ].map((exchange) => (
                        <a
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full mb-6">
                      {[
                        {
                          name: 'Uniswap',
                          url: 'https://app.uniswap.org/explore/tokens/ethereum/0xe0f63a424a4439cbe457d80e4f4b51ad25b2c56c',
                          icon: '/Uniswap_horizontallogo_pink.svg',
                          showText: false,
                        },
                        {
                          name: 'Aerodrome',
                          url: 'https://aerodrome.finance/swap?from=0x50da645f148798f68ef2d7db7c1cb22a6819bb2c&to=0x4200000000000000000000000000000000000006',
                          icon: '/velodrome-logo-light.svg',
                          showText: false,
                        },
                        {
                          name: 'Raydium',
                          url: 'https://raydium.io/swap/?outputCurrency=J3NKxxXZcnNiMjKw9hYb2K4LUxgwB6t1FtPtQVsv3KFr&referrer=6tvwZLkMWj9qZNu8YoquKNde8RJXk1fofgaAyJkWMC5f',
                          icon: '/raydium.jpeg',
                          showText: true,
                        },
                      ].map((exchange) => (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
                      {[
                        {
                          name: 'INFINEX',
                          url: 'https://app.infinex.xyz?r=1ES7E678',
                          icon: '/Infinex_Logo_Cantaloupe.svg',
                        },
                        {
                          name: 'Rabby',
                          url: 'https://rabby.io/',
                          icon: '/rabby-logo-white.svg',
                        },
                        {
                          name: 'Phantom',
                          url: 'https://phantom.com/',
                          icon: '/Phantom-Logo-Purple.svg',
                        },
                      ].map((wallet) => (
                        <a
                          key={wallet.name}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <LineChart data={holdersData}>
                      ...
                      <Tooltip
                        formatter={(value: any, name: string, props: any) => {
                          if (name === 'holders') {
                            const tooltipDataPoint = holdersData.find(
                              (d) => d.holders === value,
                            );
                            const percentChange =
                              tooltipDataPoint?.percentChange ?? 0;
                            const color =
                              percentChange < 0 ? '#ef4444' : '#22c55e';
                            return [
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#1B2236] bg-opacity-70 backdrop-blur-md p-4 rounded-xl shadow-md">
                  ...
                  {isParseHubDataLoaded &&
                  parseHubData?.['lunarcrush.com']?.sentiment ? (
                    <div className="space-y-4">
                      {(() => {
                        // Parse the HTML string to extract individual platform data
                        const htmlString =
                          parseHubData?.['lunarcrush.com']?.sentiment?.[0]
                            ?.name || '';
                        ...
                        const svgMatches = [
                          ...htmlString.matchAll(
                            /<svg[^>]*name="(twitter|youtube|tiktok|reddit)Color"[^>]*>.*?<\/svg>/g,
                          ),
                        ];
                        ...
                        svgMatches.forEach((svgMatch, index) => {
                          ...
                          const nextSvgIndex =
                            index < svgMatches.length - 1
                              ? htmlString.indexOf(svgMatches[index + 1][0])
                              : htmlString.length;

                          const platformSection = htmlString.substring(
                            svgIndex,
                            nextSvgIndex,
                          );

                          // Extract sentiment percentages from this platform section
                          const sentimentMatch = platformSection.match(
                            /width: ([\d.]+)%.*?background-color: rgb\(246, 80, 108\).*?width: ([\d.]+)%.*?background-color: rgb\(255, 132, 74\).*?width: ([\d.]+)%.*?background-color: rgb\(0, 184, 146\)/,
                          );

                          const negative = sentimentMatch
                            ? Number.parseFloat(sentimentMatch[1])
                            : 0;
                          const neutral = sentimentMatch
                            ? Number.parseFloat(sentimentMatch[2])
                            : 0;
                          const positive = sentimentMatch
                            ? Number.parseFloat(sentimentMatch[3])
                            : 0;
                          ...
                          if (platformName === 'twitter') {
                            svgContent = svgContent.replace(
                              /fill="(?:#fff|#ffffff|white|#F5FAFA)"|fill=#fff/g,
                              'fill="white"',
                            );
                            svgContent = svgContent.replace(
                              /(width="24" height="24".*?fill=["'])(#fff|#ffffff|white|#F5FAFA)(["'])/,
                              '$1white$3',
                            );
                          }
                          ...
                          >
                            ...
                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden flex-grow">
                              <div className="h-full flex">
                                <div
                                  className="h-full bg-[#f6506c] hover:brightness-125 hover:shadow-[0_0_15px_rgba(246,80,108,0.5)] transition-all duration-300"
                                ...
                                />
                                <div
                                  className="h-full bg-[#00b892] hover:brightness-125 hover:shadow-[0_0_15px_rgba(0,184,146,0.5)] transition-all duration-300"
                ...
                <div className="bg-[#1B2236] bg-opacity-70 backdrop-blur-md p-4 rounded-xl shadow-md">
                  ...
                  {/* Use isParseHubDataLoaded */}
                  {isParseHubDataLoaded &&
                  parseHubData?.['lunarcrush.com']?.engagement ? (
                    <div className="space-y-4">
                      {(() => {
                        // Parse the HTML string to extract individual platform data
                        const htmlString =
                          parseHubData?.['lunarcrush.com']?.engagement?.[0]
                            ?.name || '';
