...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate inputs
    if (!userAddress || !smartWalletAddress || !totalAmount || !frequency || !duration) {
      return NextResponse.json(
        { error: 'Missing required fields: userAddress, smartWalletAddress, totalAmount, frequency, duration' },
        { status: 400 }
      );
    }
    ...
    if (!agentKeyId && !sessionKeyApproval) {
      return NextResponse.json(
        { error: 'Either agentKeyId (for gasless) or sessionKeyApproval (legacy) is required for automated DCA orders' },
        { status: 400 }
      );
    }

    const totalAmountWei = BigInt(Math.floor(parseFloat(totalAmount) * 1e6));
    const platformFees = totalAmountWei * BigInt(Math.floor(platformFeePercentage * 100)) / 10000n;
    ...
    console.log('   Amount per order:', (Number(amountPerOrder) / 1e6).toFixed(2), 'USDC');
    console.log('   Session key approval provided:', !!sessionKeyApproval);
    console.log('   Session key approval length:', sessionKeyApproval?.length || 0);
    ...
    try {
      if (agentKeyId) {
        ...
        agentKey = await serverAgentKeyService.generateAgentKey(userAddress as Address);
      ...
      throw new Error(`Agent key handling failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    ...
    try {
      ...
      // Timestamps
      createdAt: now,
      nextExecutionAt: now + frequencyMs, // First execution after initial delay
    ...
    try {
      ...
      if (contractResult.success) {
        console.log('✅ Order registered with Gelato contract:', contractResult.orderId);
        console.log('   Transaction hash:', contractResult.txHash);
      } else {
        console.warn('⚠️ Failed to register with contract (order still created):', contractResult.error);
      }
    } catch (contractError) {
      console.warn('⚠️ Contract registration error (order still created):', contractError);
    ...
    return NextResponse.json({
      ...
      order: {
        ...
        duration: duration,
        totalExecutions: totalExecutions,
        amountPerExecution: (parseFloat(totalAmount) / totalExecutions).toFixed(2),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const orders = await serverDcaDatabase.getUserOrders(userAddress as Address);
    ...
    const formattedOrders = v2Orders.map(order => {
      ...
      return {
        ...
        nextExecutionAt: order.nextExecutionAt ? new Date(order.nextExecutionAt).toISOString() : null,
