...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      slippagePercentage = 0.05, // Increased to 5% for better execution success
    } = body;
    ...
    const params = new URLSearchParams({
      ...
      slippage: (slippagePercentage * 100).toString(), // Percentage format (5 for 5%)
    ...
    console.log('🌊 OpenOcean v4 Parameters:', {
      ...
      slippage: (slippagePercentage * 100).toString(),
      account: takerAddress,
      receiver: receiverAddress || takerAddress,
    });
    ...
    if (data.code !== 200 || !data.data) {
      ...
      throw new Error(
        `OpenOcean v4 API error: ${data.error || data.message || 'Unknown error'}`,
      );
    }
    ...
    console.log(
      `   📥 Input: ${humanInputAmount.toFixed(6)} ${swapData.inToken?.symbol || 'tokens'}`,
    );
    console.log(
      `   📤 Output: ${humanOutputAmount.toFixed(6)} ${swapData.outToken?.symbol || 'tokens'}`,
    );
    console.log(
      `   💰 Rate: 1 ${swapData.inToken?.symbol} = ${(humanOutputAmount / humanInputAmount).toFixed(2)} ${swapData.outToken?.symbol}`,
    );
