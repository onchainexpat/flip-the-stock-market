...
async function createSessionKeyWithPermissions(
  ...
  console.log('Matches original wallet?', permissionAccount.address === ownerAccount.address);
  
  // Serialize the permission account WITH the session private key
  const serializedSessionKey = await serializePermissionAccount(permissionAccount, sessionPrivateKey);
...
async function runSessionKeyTest() {
  ...
  try {
    ...
    const { owner, account, kernelClient, publicClient } = await deploySmartWallet(ownerPrivateKey);
    
    // Check if wallet needs funding
    const balance = await publicClient.getBalance({ address: account.address });
    console.log('Smart Wallet ETH Balance:', balance.toString(), 'wei');
    if (balance === 0n) {
      console.log('⚠️  Smart wallet has no ETH. Send some Base Sepolia ETH to:', account.address);
      console.log('Note: With ZeroDev paymaster, gas will be sponsored so ETH is not required for transactions.');
    }
    ...
    console.log('2. The agent can execute transactions without additional signatures');
...
export { runSessionKeyTest, deploySmartWallet, createSessionKeyWithPermissions, agentUsesSessionKey };