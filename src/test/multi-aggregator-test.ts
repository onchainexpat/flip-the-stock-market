...
async function testMultiAggregatorQuotes() {
  ...
  try {
    ...
    console.log(`Average response time: ${result.metadata.averageResponseTime}ms`);
    console.log(`Savings: ${result.savingsVsWorst.amount} tokens (${result.savingsVsWorst.percentage}%)\n`);
...
async function testExecutableSwapData() {
  ...
  try {
    ...
    console.log(`Transaction data length: ${result.bestSwap.data.length} chars`);
    console.log(`Gas estimate: ${result.bestSwap.gas}`);
    console.log(`Price impact: ${result.bestSwap.priceImpact}%`);
    console.log(`Successful quotes: ${result.executionMetadata.successfulQuotes}/${result.executionMetadata.totalAggregators}`);
    console.log(`Savings: ${result.savings.amount} tokens (${result.savings.percentage}%)\n`);
    
    // Validate the swap data
    const validation = aggregatorExecutionService.validateSwapData(result.bestSwap);
...
async function testErrorHandling() {
  ...
  try {
    ...
    console.log(`Quotes received with small amount: ${result.metadata.quotesReceived}`);
    console.log(`Best aggregator handled small amount: ${result.bestQuote.aggregator}`);
...
async function testSPXPriceComparison() {
  ...
  try {
    ...
    priceResult.allPrices.forEach(price => {
      console.log(`  ${price.aggregator}: $${price.price} (${price.responseTime}ms)`);
    });
