...
async function createETHSwapSessionKey(
  ...
  if (balance < parseEther('0.005')) {
    console.log('⚠️  Low ETH balance. May not be sufficient for multiple swaps.');
  }
...
async function runETHToUSDCTest() {
  ...
  try {
    ...
    console.log('Slippage Tolerance:', TEST_CONFIG.slippageTolerance / 100, '%');
    ...
    const swapResults = await testETHToUSDCSwaps(serializedSessionKey, smartWalletAddress);
