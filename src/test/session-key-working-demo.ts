...
async function createTransferSessionKey(
  ...
  const balance = await publicClient.getBalance({ address: originalAccount.address });
...
async function runWorkingDemo() {
  ...
  try {
    ...
    console.log('Per Transfer:', formatEther(TEST_CONFIG.transferAmount), 'ETH');
    ...
    console.log('Failed transfers:', TEST_CONFIG.totalTransfers - transferResults.length);
    
    if (transferResults.length > 0) {
      console.log('\n🔗 Transaction Links:');
      transferResults.forEach((result, i) => {
        console.log(`Transfer ${i + 1}: https://sepolia.basescan.org/tx/${result.txHash}`);
      });
      
      const totalGasUsed = transferResults.reduce((sum, r) => sum + r.gasUsed, 0n);
      console.log('\n⛽ Gas Summary:');
      console.log('Total gas used:', totalGasUsed.toString());
      console.log('Average per transfer:', (totalGasUsed / BigInt(transferResults.length)).toString());
    }
    ...
    console.log('3. ✅ Multiple transactions executed without user interaction');
    console.log('4. ✅ Address preservation works (same wallet, different permissions)');
    console.log('5. 🔄 For DCA: Replace ETH transfers with token swaps when liquidity is available');
