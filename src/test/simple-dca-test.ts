...
async function testCompleteDCAFlow(): Promise<void> {
  ...
  try {
    ...
    const wallet = await simpleDCAService.createSimpleSmartWallet(TEST_CONFIG.USER_WALLET);
    ...
    const userSPXBalance = await simpleDCAService.getSPXBalance(wallet.userWalletAddress);
    
    console.log(`💰 Smart wallet USDC balance: ${(Number(usdcBalance) / 1e6).toFixed(6)} USDC`);
    console.log(`💰 User wallet SPX balance: ${(Number(userSPXBalance) / 1e18).toFixed(6)} SPX\n`);
    ...
    if (usdcBalance < TEST_CONFIG.MINIMUM_BALANCE_REQUIRED) {
      ...
      console.log(`   2. Send at least ${(Number(TEST_CONFIG.MINIMUM_BALANCE_REQUIRED) / 1e6).toFixed(6)} USDC to this address`);
    ...
    if (swapResult.success) {
      ...
      const newUSDCBalance = await simpleDCAService.getUSDCBalance(wallet.address);
      const newUserSPXBalance = await simpleDCAService.getSPXBalance(wallet.userWalletAddress);
      
      console.log('\n📊 Final Balances:');
      console.log(`💰 Smart wallet USDC: ${(Number(newUSDCBalance) / 1e6).toFixed(6)} USDC (was ${(Number(usdcBalance) / 1e6).toFixed(6)})`);
      console.log(`💰 User wallet SPX: ${(Number(newUserSPXBalance) / 1e18).toFixed(6)} SPX (was ${(Number(userSPXBalance) / 1e18).toFixed(6)})`);
      
      const usdcSpent = usdcBalance - newUSDCBalance;
      const spxReceived = newUserSPXBalance - userSPXBalance;
      
      console.log('\n📈 Transaction Summary:');
      console.log(`💸 USDC spent: ${(Number(usdcSpent) / 1e6).toFixed(6)} USDC`);
      console.log(`📈 SPX received: ${(Number(spxReceived) / 1e18).toFixed(6)} SPX`);
      
      if (spxReceived > 0) {
        console.log('🎉 DCA swap successful - SPX tokens delivered to user wallet!');
      } else {
...
async function checkBalances(walletAddress: Address): Promise<void> {
  ...
  try {
    ...
    if (usdcBalance >= TEST_CONFIG.MINIMUM_BALANCE_REQUIRED) {
      ...
      console.log(`⚠️ Need ${(Number(TEST_CONFIG.MINIMUM_BALANCE_REQUIRED - usdcBalance) / 1e6).toFixed(6)} more USDC for testing`);
    }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'balance':
      if (args[1]) {
        ...
        console.log('❌ Please provide wallet address: bun run simple-dca-test.ts balance <address>');
      }
      break;
    case 'full':
    default:
  ...
  console.log('\n📋 Available Commands:');
  console.log('  bun run src/test/simple-dca-test.ts full     # Test complete DCA flow');
  console.log('  bun run src/test/simple-dca-test.ts wallet   # Create wallet only');
  console.log('  bun run src/test/simple-dca-test.ts balance <address>  # Check balances');
}
