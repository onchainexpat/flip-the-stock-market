...
async function createETHTransferSessionKey(
  ...
  if (balance < TEST_CONFIG.maxDailyLimit) {
    console.log('⚠️  Insufficient balance for testing. Need at least', formatEther(TEST_CONFIG.maxDailyLimit), 'ETH');
...
async function testValueLimits(serializedSessionKey: string) {
  ...
  try {
    ...
    console.log('❌ Value limit test failed - transaction should have been rejected');
    return false;
...
async function runETHTransferTest() {
  ...
  try {
    ...
    console.log('Daily ETH Limit:', formatEther(TEST_CONFIG.maxDailyLimit), 'ETH');
    ...
    // Summary
    console.log('\n📊 Test Results:');
    console.log('Successful transfers:', transferResults.filter(r => r.success).length);
    console.log('Failed transfers:', transferResults.filter(r => !r.success).length);
