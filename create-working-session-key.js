...
async function createWorkingSessionKey() {
  ...
  try {
    ...
    if (result.success) {
      ...
      const testResponse = await fetch(`http://localhost:3000/api/test-force-dca-execution?orderId=${result.orderId}`);
      
      if (testResponse.ok) {
        const testResult = await testResponse.json();
        console.log('✅ Execution test result:', testResult.success ? 'SUCCESS' : 'FAILED');
