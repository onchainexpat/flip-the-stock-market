...
async function testContractService() {
  try {
    ...
    const { dcaGelatoContractService } = await import(
      './src/services/dcaGelatoContractService.ts'
    );
    ...
    console.log(
      '   Wallet Address:',
      contractInfo.walletAddress || 'Not configured',
    );
    ...
    console.log(
      ...
      process.env.GELATO_DEPLOYER_PRIVATE_KEY ? 'Set' : 'NOT SET',
    );
    console.log(
      '   DCA_RESOLVER_ADDRESS:',
      process.env.DCA_RESOLVER_ADDRESS || 'NOT SET',
    );
    console.log(
      '   NEXT_PUBLIC_ZERODEV_RPC_URL:',
      process.env.NEXT_PUBLIC_ZERODEV_RPC_URL ? 'Set' : 'NOT SET',
    );

    if (!process.env.GELATO_DEPLOYER_PRIVATE_KEY) {
      console.log('\n⚠️  WARNING: GELATO_DEPLOYER_PRIVATE_KEY not set!');
      console.log(
        '   This means the contract service cannot write to the contract.',
      );
      console.log(
        '   Orders will be created in the database but not registered in the smart contract.',
      );
    }
  } catch (error) {
