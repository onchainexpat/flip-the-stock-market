...
async function fundWorkingWallet() {
  ...
  try {
    ...
    console.log('   Existing wallet:', (Number(existingBalance) / 1e6).toFixed(6), 'USDC');
    ...
    console.log('   Working wallet:', (Number(workingBalance) / 1e6).toFixed(6), 'USDC');
    ...
    console.log('🔄 Transferring 1 USDC from existing wallet to working wallet...');
    console.log('   This requires using the session key of the existing wallet');
    
    // Get the latest existing order to use its session key
    const ordersResponse = await fetch(`${BASE_URL}/api/unified-dca-orders?userAddress=0xC9860f5D7b80015D0Ff3E440d0f8dB90A518F7E7&limit=1`);
    ...
    if (transferResult.success) {
      ...
      console.log('📊 New working wallet balance:', (Number(newWorkingBalance) / 1e6).toFixed(6), 'USDC');
